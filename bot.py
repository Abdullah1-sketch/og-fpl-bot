...
def _prediction_alert_post(player, direction):
    pct = abs(float(player["prediction"]))
    marker = "🔥" if pct >= 95 else "⚠️"
    arrow = "⬆️" if direction == "rise" else "⬇️"
    label = "احتمال ارتفاع" if direction == "rise" else "احتمال انخفاض"
    return "\n".join([
        "🚨 توقع تغيّر السعر",
        "",
        f"{arrow} {label}",
        f"{marker} {player['name']} — {pct:.0f}%",
        "",
        "#FPL",
        "#فانتزي_البريميرليغ",
    ])
...
def maybe_post_prediction_crossings(bootstrap, state):
    """Post immediately when an official FPL projection first reaches ±threshold.

    A player is posted only once in the current official price-change cycle.
    Alerts are re-armed only after an official price update is detected, not merely
    because the live percentage dipped below 85% and crossed it again.
    """
    ...
    risers = sorted(
        [p for p in rows if p["prediction"] >= PREDICTION_THRESHOLD and str(p["id"]) not in alerted_rise],
        key=lambda p: p["prediction"], reverse=True,
    )
    fallers = sorted(
        [p for p in rows if p["prediction"] <= -PREDICTION_THRESHOLD and str(p["id"]) not in alerted_fall],
        key=lambda p: p["prediction"],
    )

    for p in risers:
        if safe_post(_prediction_alert_post(p, "rise"), "price prediction"):
    ...
    for p in fallers:
        if safe_post(_prediction_alert_post(p, "fall"), "price prediction"):
...
def check_and_post_confirmed_prices(bootstrap, state):
    ...
    if old_prices:
        ...
        if rises or falls:
            ...
            changed = True
            # A confirmed official price update starts a new prediction cycle.
            state["alerted_rise"] = {}
            state["alerted_fall"] = {}
...
def run_once():
    ...
    # Official FPL predictor: alert immediately when a player crosses ±85%.
    try:
