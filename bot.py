...
def _prediction_direction_post(players, direction):
    ...
    if direction == "rise":
        ...
        lines += [f"⬆️ {p['name']} — {abs(float(p['prediction'])):.0f}%" for p in players]
    else:
...
def maybe_post_prediction_crossings(bootstrap, state):
    """Post newly qualifying official FPL projections at ±threshold.
    ...
    re-armed only after an official price update is detected.
    """
