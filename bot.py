    ...
    for row in live['elements']:
        ...
        for part in row.get('explain', []):
            ...
            if players[pid].get('team') not in (fixture['team_h'], fixture['team_a']):
                continue
            values = {'goals_scored': 0, 'assists': 0}
...
def match_post(fixture, changes, players, teams):
    """Do not assert a goal-assist pairing from aggregate counts."""
    ...
    for change in changes:
        ...
        if change['delta'] > 0:
            label = '⚽️ هدف' if goal else '🅰️ صناعة'
            suffix = '' if change['delta'] == 1 else f" ×{change['delta']}"
            # Distinguish a player's second/third goal without inventing match minutes.
            ordinal = f" (رقم {change['new']})" if change['new'] > 1 and change['delta'] == 1 else ''
            lines.append(f'{label} | {name}{suffix}{ordinal}')
        else:
            label = 'الأهداف' if goal else 'الصناعة'
            lines.append(f"⚠️ تصحيح FPL | {name}: {label} المحتسبة {change['new']}")
    return '\n'.join(lines + ['', '#FPL', '#فانتزي_البريميرليغ'])


def process_matches(fixtures, live_by_gw, bootstrap, state):
    ...
    for fixture in fixtures:
        ...
        # One official credited goal + one assist on the FIRST goal of the match
        # is unambiguous only when both scoreboards also show exactly one goal.
        goals = [c for c in changes if c['stat'] == 'goals_scored']
        assists = [c for c in changes if c['stat'] == 'assists']
        first_pair = (len(changes) == 2 and len(goals) == len(assists) == 1
                      and all(c['old'] == 0 and c['new'] == 1 for c in changes)
                      and sum(v['goals_scored'] for v in previous.values()) == 0
                      and sum(v['goals_scored'] for v in current.values()) == 1
                      and (fixture.get('team_h_score') or 0) + (fixture.get('team_a_score') or 0) == 1
                      and players[goals[0]['pid']]['team'] == players[assists[0]['pid']]['team'])
        batches = [goals + assists] if first_pair else [[c] for c in changes]
        for batch in batches:
            ...
            # Reserve BEFORE X call. If the response is lost, do not blindly
            # retry a potentially already-published tweet after a restart.
            state['match_revision'] = revision
            for c in batch:
                previous[c['pid']][c['stat']] = c['new']
            outbox[key] = {'status': 'pending', 'text': post,
